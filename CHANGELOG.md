...
- Imported **character cards** (V1, V2, and V3, as JSON or the PNG they come in, including compressed ones) through **IMPORT CHARACTER**. The name and short personality go onto the sheet, the card's image becomes the portrait, and its description, scenario, and example dialogue become the character's reference profile, which the engine treats as reference rather than instructions. `{{char}}` and `{{user}}` are filled in, a V3 nickname is honored, and the card's lorebook joins the Lore tab when the character is saved. The card's system prompt, post-history instructions, and greetings are left out, and the import says so.
